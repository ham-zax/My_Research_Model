...
The exact screening margin is intentionally not chosen until the acquisition-day sensitivity calculation completes. Simply purchasing only days where Binance itself crosses exactly -1% could miss some final composite events. Across the 39 available development quote-grid days from July 2023 onward, 3,357,829 valid five-minute two-venue comparisons were available. The dangerous one-sided error is Binance return minus composite return when the composite is falling: among the 1,506 seconds where the composite five-minute return was already <= -1%, the largest positive difference was 31.981 bps. At that second the Binance-only return was -70.390 bps while the equal-weight Binance/Bybit composite was -102.372 bps. Therefore a -0.7% Binance screen only barely covers the worst development discrepancy observed so far; -0.65% or -0.60% provides actual headroom. A separate audit of the first 92 sealed segments of the current forward capture produced 7,481 valid simultaneous spot seconds and 7,163 valid five-minute return comparisons, with at most 2.516 bps absolute Binance-versus-composite return difference. The continuous result is reassuring but too short to replace the broader development evidence. The chosen screening threshold must be frozen before any new paid L2 outcome/model inspection.
