...
**Latest operational update:** the development process later stopped without a terminal marker; its last checkpoint was 03:59:45 UTC. The [interruption record](../artifacts/e001_capture_interruption_observation.json) documents unchanged preservation of the unclean tail. At about 05:33 UTC, a fresh 60-second recovery capture passed integrity but produced zero valid composite seconds under the original clock gate. Bybit median receipt-minus-source lag was -2 ms, with minimum -5 ms on spot and -4 ms on linear. Windows reported synchronized; this small cross-clock disagreement still fails the existing strict rule. [Recovery replay evidence](../artifacts/e001_replay_recovery_smoke_results.json). Development recording remains stopped. The earlier successful check below applies only to its observed interval.

The first recurring envelope discrepancy in the sealed development prefix occurred at **01:43:09.299668 UTC**: a Bybit spot timestamp exceeded local receipt by only **0.331859 ms**. [Exact record evidence](../artifacts/e001_first_recurring_clock_failure.json). Under the current implementation, any such discrepancy blocks the remainder of the run. This exposes a limitation of the strict cross-clock comparison; it does not establish literal negative network latency. An explicit uncertainty/availability policy must be designed, verified and versioned before changing that behavior. No tolerance has been chosen from outcomes or silently applied here.
...
Supported receipt-aware trade/liquidation-size windows, persistent book additions, pre-trigger history and neutral feature transforms are now [implemented](e001_live_features.md). The full panel remains gated by unsupported economics, missing depth coverage, and recurring clock disagreement. Preserve the stopped run's clean prefix separately from its unclean tail. Do not route the new capture envelope directly into the fixture adapter, infer full depth from partial books, or value liquidations at bankruptcy prices. Keep model fitting disabled until the common feature panel and independently eligible development sample are sufficient.
