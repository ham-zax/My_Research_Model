...
**Selected-row and capacity audits done; feature contract unresolved:** see [field matrix](e001_feature_feasibility.md) and [capacity audit](../artifacts/e001_capacity_audit_report.md). Six diagnostic live decisions are insufficient to establish a supported primary comparison. The downloaded Tardis data also cannot reconstruct the prescribed 25 bps capacity proxy.
...
- [ ] Produce a field matrix: economic meaning, units, source, observable status, missingness, neutral B4 input and any dependent MFSM combination. Distinguish measured zero from unavailable data.
...
- [ ] Resolve the common feature list before real fitting. Do not require every diagnostic column merely because it exists, and do not impute an economically undefined required feature or fit a purported primary MFSM comparison with its defining interactions absent.
- [ ] If a narrower observable comparison is proposed, list exactly which claims/features are removed or changed, give both learners the same neutral data, and obtain the material contract decision before selecting it as a primary run. Record the choice before model scores are available.
...
**Do not iterate the band again on these outcomes.** The next observable candidate should remove the literal capacity-limit claim and use a prespecified fixed-level liquidity state normalized by its own historical scale. Its empirical evaluation needs fresh dates or a separately fixed continuous archive. The 42 dates used here may support engineering tests but are now development data for this feature family.
...
- [ ] Create a data-sufficiency check against the prespecified split/calibration design. Both classes overall are necessary but insufficient: required fitting/calibration partitions and independent evaluation periods must be usable. The two known monthly-sample negative labels cannot pass.
