...
**Historical extension selected for feasibility:** use the [CryptoStruct candidate path](e001_cryptostruct_historical_data.md) to solve the sample-size problem without changing the frozen E001-LIQ-OBS feature contract. The consecutive official Binance BTCUSDT one-second scan is complete for July 2023-August 2026: all 38 monthly checksums passed, 100,051,200 seconds had zero gaps, and the price-only proxy found 428 accepted two-hour-lockout episodes on 283 days. The deterministic paid-data candidate-day rule is frozen in `artifacts/e001_historical_screen_calibration.json`: retain a day for any valid free Binance+Bybit-trade five-minute return <= -0.60%, or when that free reference is unavailable and Binance alone is <= -0.50%; apply no screening lockout. The full frozen acquisition screen has now also completed: 748 candidate days and 775 spot seed/support days are sealed in `artifacts/e001_historical_spot_acquisition_manifest.json`; 12 early support/candidate days are outside Bybit spot catalog coverage. A read-only CryptoStruct quote found all 1,526 purchasable Binance+Bybit spot instrument-days available with zero missing at EUR 1,526 total. No checkout occurred. The final research trigger still must be rebuilt from paid Binance+Bybit spot receipt-time midquotes before any Bybit perpetual L2 purchase or real model fit. The free Bybit perpetual sample passed the initial top-50/gap/timing probe and the source adapter can produce the frozen liquidity feature vector.
...
- [ ] Distinguish that qualification check from a research dataset. The current 20,673-second E001-LIQ-OBS-1 seed capture can contain at most three independent episodes under the fixed 7,200-second lockout, so it cannot satisfy the frozen 130-episode evaluation gate. Even without unknown lockout history, 1,815 seconds of feature warm-up plus a full 1,800-second primary label horizon already require over an hour for the earliest matured decision. Any longer evaluation acquisition window must be fixed before launch and before inspecting its labels or model scores.
